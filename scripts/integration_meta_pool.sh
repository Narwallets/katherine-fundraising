...
NEAR_ENV=testnet near dev-deploy --wasmFile res/test_p_token.wasm --initFunction new_default_meta --initArgs '{"owner_id": "'$NEAR_ACCOUNT'", "total_supply": "1000'$YOCTO_UNITS'", "symbol": "pToken", "decimals": 24}'
mv neardev/ neardev_ptoken/
NEAR_ENV=testnet near dev-deploy --wasmFile res/test_p_token.wasm --initFunction new_default_meta --initArgs '{"owner_id": "'$NEAR_ACCOUNT'", "total_supply": "1000'$BEAR_UNITS'", "symbol": "BEAR", "decimals": 6}'
mv neardev/ neardev_ptoken_bear/

METAPOOL_CONTRACT_ADDRESS=$(head -n1 ./neardev_metapool/dev-account)
PTOKEN_CONTRACT_ADDRESS=$(head -n1 ./neardev_ptoken/dev-account)
BEAR_CONTRACT_ADDRESS=$(head -n1 ./neardev_ptoken_bear/dev-account)

NEAR_ENV=testnet near dev-deploy --wasmFile res/katherine_fundraising_contract.wasm --initFunction new --initArgs '{"owner_id": "'$NEAR_ACCOUNT'", "min_deposit_amount": "1'$YOCTO_UNITS'", "metapool_contract_address": "'$METAPOOL_CONTRACT_ADDRESS'", "katherine_fee_percent": 200 }'
...
echo "BEAR: "$BEAR_CONTRACT_ADDRESS
