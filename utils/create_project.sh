...
if [[ "${1}" != "" ]]; then
	...
	echo "create_project.sh testnet project_pembrock.conf"
fi
...
PROJECT_ID=$(NEAR_ENV=testnet near call $KATHERINE_CONTRACT_ADDRESS get_kickstarter_id_from_slug '{"slug": "'$PROJECT_SLUG'"}' --accountId $KATHERINE_OWNER_ID | grep "https://explorer.testnet.near.org/transactions/" -A 1 | grep -v "https://explorer.testnet.near.org/transactions")
