...
impl FungibleTokenReceiver for KatherineFundraising {
    fn ft_on_transfer(
        ...
        let result = if env::predecessor_account_id() == self.metapool_contract_address {
            ...
            self.internal_supporter_deposit(sender_id.as_ref(), &amount.0, &kickstarter_id)
        } else {
...
mod tests {
    ...
    ) -> (VMContextBuilder, KatherineFundraising) {
    ...
    fn add_supporter_with_ext_callback() {
        ...
        let kickstarter_id = ctr.create_kickstarter("first_kickstarter".to_owned(), "FK".to_owned(), kickstarter_owner.to_string(), get_time_millis(&ctx), get_time_millis(&ctx) + 1_000*60*5, kickstarter_token_acc.to_string());
        ...
        assert_eq!(kickstarter_data.total_supporters, 1, "incorrrect number of supporters for kickstarter");
    }
}