...
use crate::{constants::*, k1::*, supporter::*, types::*};

// The KatherineFundraising Contract would be managed by K2 under the k1 standard struct `StandardK1Contract`.

#[near_bindgen]
#[derive(BorshDeserialize, BorshSerialize, PanicOnDefault)]
pub struct K2therineContract {
    pub owner_id: AccountId,
    
    // The box contains an standard version of the K1therine Contract.
    pub k1_box: Option<Vector<StandardK1Contract>>,
...
impl K2therineContract {
    ...
    pub fn new(
        ...
        // TODO: assert!(!env::state_exists(), "The contract is already initialized");
        Self {
            owner_id,
            k1_box: None,
    ...
    // TODO: K1 CRUD
    pub fn create_k1(&mut self) {
        let k1 = StandardK1Contract::new(
            self.owner_id,
            self.min_deposit_amount,
            self.metapool_contract_address,

            // TODO: this code is only for testing DEFAULT, please update.
            BasisPoints::from(200),
        );

        let mut k1_box = self.k1_box.unwrap_or(Vector::new::<StandardK1Contract>("K1BOX"));
        k1_box.push(k1);
        self.k1_box = Some(k1_box);
    }

    // Generic method to call functions in K1.
    pub call_direct_k1_method(
        &mut self,
        k1_index: u64,
        method_name: String,
        arguments: String
