...
fn new_contract() -> StandardK1Contract {
    StandardK1Contract::new(
        get_katherine_owner(),
...
fn get_contract_setup(context: VMContext) -> StandardK1Contract {
    testing_env!(context.clone());
    let contract = new_contract();
    contract
}

fn create_kickstarter_with_2_goals(contract: &mut StandardK1Contract, now: Now) -> KickstarterId {
...
fn get_ready_to_funded_kickstarter_contract(now: Now) -> StandardK1Contract {
...
fn deposit_kickstarter_tokens(
    contract: &mut StandardK1Contract,
